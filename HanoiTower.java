...
public class HanoiTower {
	 ...
	 * 	@param	int		number of levels on the leftmost ring
	 */
	public void recurseSolve(int leftLevel) {
		//	Base case
		if (leftLevel == 0) return;
		
		//	Keep track of pegs to move to
		//	Peg to try to shift the top triangle to
		int mainPeg = 0;
		//	Peg that will be empty by the call of next method
		int sidePeg = 0;
		//	Set pegs
		if (leftLevel % 2 == 0) {
			mainPeg = 2;
		...
		//	First move, sets bottom of new tower
		moveDisk(0, mainPeg);
		printTowers();
		
		//	Move new tower from side to main
		boolean finished = (tower[sidePeg][0] == 0);
		int sideLevels = levels - leftLevel - 1;
		while (!finished) {
			if (moveDisk(sidePeg, mainPeg));
			else if (moveDisk(0, mainPeg));
			else if (moveDisk(mainPeg, 0));
			//else if (moveDisk(sidePeg, 0));
			printTowers();
			finished = (tower[sidePeg][0] == 0 && tower[mainPeg]
			[levels - leftLevel] != 0);
		}
		
		//	Move next
		recurseSolve(leftLevel - 1);
	}
